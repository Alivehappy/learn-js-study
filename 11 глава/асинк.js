...
///fetch — это асинхронная функция, которая сразу возвращает промис (не дожидаясь ответа сервера). Без await вы получите не сам ответ, а лишь обещание (промис), что ответ когда-то появится
{
	async function shhowAvatar() {
...
//Фактически это искусственная пауза в выполнении кода
...
//Промис может быть разрешён с ЛЮБЫМ значением, включая NaN
...
Такие ошибки можно ловить, используя try..catch, как с обычным <throw:></throw:>*/
{
	async function f() {
		try {
...
//ивиди кпро прмисы и прочитать еще раз
//2 части кнспект
//версиаем час
...
/*Сетевой запрос
fetch получает "сырой" HTTP-ответ, включая заголовки и бинарное тело.

Объект Response
Возвращается объект Response, который ещё не содержит распарсенных данных. Это просто обёртка над бинарным потоком.

Методы для чтения тела
Чтобы получить полезные данные, нужно вызвать один из методов:

.text() → string

.json() → object (парсит JSON)
