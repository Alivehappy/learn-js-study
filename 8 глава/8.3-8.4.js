...
//name добавляется только экземплярам:
//__proto__ этого объекта ссылается на Rabbit.prototype

//Прототип содержит только:

//Методы (если вы их добавили, например Rabbit.prototype.jump = ...)

//Cтандартные свойства прототипа (constructor и др.)
console.log(Rabbit.prototype.hasOwnProperty('name')); // false
/*Rabbit.prototype — это "шаблон" для создания экземпляров

Свойства, заданные через this. в конструкторе, попадают в экземпляры

Прототип содержит только общие методы (для экономии памяти)

Поэтому когда вы видите console.log(Rabbit.prototype) — там действительно нет name, так как это свойство создаётся отдельно для каждого нового объекта.
*/
